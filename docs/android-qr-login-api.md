...
Operators must rate-limit claim/exchange by source at ingress and avoid body/query logging. The application also enforces a five-second minimum exchange interval, caps all unexpired attempt rows before QR rendering, applies short expiry and bounded pruning, and returns generic terminal errors. Audit events cover create, claim, approve, deny, expiry, and exchange outcomes without secret values.
