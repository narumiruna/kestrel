...
The backend uses Authorization Code flow with PKCE, verifies discovery issuer, ID-token signature, audience, expiry, OIDC nonce, and subject, and then redirects clients with a short-lived one-time Kestrel exchange ticket in the URL fragment so Web servers do not receive it. The ticket is bound to a secret retained by the initiating Web tab or Android app and consumed atomically. Pocket ID tokens and Kestrel session tokens never appear in callback URLs.
