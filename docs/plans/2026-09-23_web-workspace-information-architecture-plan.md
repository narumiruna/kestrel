...
- 本計畫依使用者的 UI review 明確納入 Desktop／Tablet／Mobile responsive behavior 與 `Light | Dark | System` appearance，因此只對這些路徑覆寫 Web PoC 的 1440×900 light-only validation 預設。1440×900 light mode 仍是主要 baseline；額外驗證限於代表性 tablet／mobile viewport 與 appearance modes，不擴張成其他 viewport、zoom 或 RTL matrix。
