...
- [ ] 依 PR 邊界交付以降低回歸：先 shared chrome + Account menu/theme，次 Account route split，再 Map panel/safe-area/editor polish，最後 Library master-detail/cross-flow + docs；每個 PR 都從當時最新 main 建 branch、只含該 slice、通過受影響 checks，且不得用未完成後續 PR 掩蓋該 slice 的 broken state。
