...
- 本計畫依使用者的 UI review 明確納入 Desktop／Tablet／Mobile responsive behavior 與 `Light | Dark | System` appearance，因此只對這些路徑覆寫 Web PoC 的 1440×900 light-only validation 預設。1440×900 light mode 仍是主要 baseline；額外驗證限於代表性 tablet／mobile viewport、appearance modes，以及 panel persistence 所需的一個 1100×900 desktop-breakpoint case，不擴張成其他 viewport、zoom 或 RTL matrix。
...
- **Library** 改為 URL-addressable master-detail catalog。query、type、sort、filter 與 selected ID 由 search params 擁有，桌面顯示 list + read-only preview，窄螢幕在 list／detail 間切換；`Open on map` 才進入 canonical editor。selected ID 只有在目前 filtered result 中才有效；不相容、缺失或已刪除的 selection 必須從 canonical URL 移除並顯示未選取 preview，不可保留 off-list item 或默默選第一筆。既有 `/dashboard/library/places`、`/dashboard/library/routes` 與 legacy redirects 保持相容，並正規化到同一份 canonical URL state。
- **Account** 使用共用 account layout 與獨立 routes：`/dashboard/account`（read-only overview）、`/dashboard/account/appearance`、`/dashboard/account/security`、`/dashboard/account/sessions`。`/dashboard/account/oidc` 保留為專用 OIDC link callback／exchange boundary，`/dashboard/account?oidc=linked` 保留成功 notice 相容性；Header account menu 只保留導覽、compact `system | light | dark` control 與 Sign out，不提交 password 或其他高風險資料。所有 Account route entry 與 Sign out 都必須沿用 Map 的 draft action guard；不可用 client-side `Link` 或先清 session 來繞過未儲存變更確認。
...
- **Panel state** 由一個可測試的 local preference model 擁有 desktop collapsed state 與 widths，輸入必須 versioned、clamped 且能在 unavailable Web Storage 下安全降級。Desktop effective widths 必須依目前 viewport、固定 gutters 與至少 300 CSS px 的 unobscured map workspace 進行 joint clamp，viewport 改變時重新計算；若超出總預算，在各自 minimum 以上按比例縮減兩側。窄於 1024px 時同時忽略 persisted widths 與 collapsed flags、不得覆寫保存的 desktop preference，回到 desktop 時才恢復。
...
- [ ] 抽出並以 Node tests 鎖定 workspace presentation helpers：manual-save label、Library search-param parsing/serialization／filtered-selection normalization、sort/filter comparators、panel preference versioning／viewport-aware joint clamping，以及 panel-aware map padding；測試 malformed URL/storage、unknown enum、selected ID 被 query/type/tag/mode 排除、missing/deleted ID、empty results、sort-only change、unavailable storage、極端單側／雙側 widths、1024／1100／1150／1440 viewport budgets、left/right/both collapsed 從 desktop 進出 narrow contextual layout，以及 preference restore。
...
- [ ] 將 `WorkspaceHeader` account popover 改成小型 Radix menu，只呈現 username、`View account`、compact `Appearance` mode control／settings entry 與 `Sign out`；移除 password form、API mutation與大型 popover state，並驗證 Escape／outside click／route navigation 的 focus return。`View account`、`Appearance settings` 與未來所有 account-menu route entry 都必須呼叫既有 `onBeforeWorkspaceChange`／`navigateIfDraftSafe` contract；`Sign out` 必須先呼叫 `requestDraftAction`，只有 clean draft 或使用者確認後才執行一次 `auth.logout`／session revocation。因 menu item 會在 selection 後 unmount，guard contract 必須接受 account-menu trigger 的 stable ref 作為 restore-focus fallback；dialog cancel 時若原 active element 已不在 DOM，將 focus 回 trigger。tests 覆蓋 dirty draft cancel（不導覽、不清 draft、不登出且 `document.activeElement` 是 trigger）與 route／Sign out confirm（各動作只執行一次）。`Appearance settings` link 與可用的 `/dashboard/account/appearance` page 必須在同一 PR slice 交付，不可合併指向未建立 route 的 menu。
...
- [ ] 在 `Stage` 與 Map page 加入 desktop-only accessible panel resizing：pointer drag handle 必須可 focus，使用 `role="separator"`、`tabIndex={0}`、`aria-orientation="vertical"`、可讀 label、`aria-controls`，並以 `aria-valuenow`／`aria-valuemin`／`aria-valuemax` 暴露目前與 effective clamped width range；方向鍵提供 keyboard resize、double-click/reset 回復 defaults。保存 desktop left/right widths 與 collapse state，每次 load／viewport resize 都 joint clamp 兩側並保留至少 300 CSS px unobscured map workspace；窄於 1024px 時忽略兩側 desktop widths／collapsed flags且不改寫 preference，讓 Map／Choose／Edit contextual layout 擁有 visibility，返回 desktop 再恢復保存狀態。keyboard／screen-reader 驗證必須證明 handle 可發現、width value 會被宣告且調整後同步更新；browser tests 覆蓋 left/right/both collapsed desktop → tablet/mobile → desktop 與 1024–1150px restore。
...
- [ ] 重新定義 Map picker 為 Library quick picker：加入簡短 scope copy與 `Manage in Library` link，保留 kind-specific `New place`／`New route`，提供最近更新／名稱排序但不複製完整 Library filters；用非完成語意的 selected indicator、accent line、`aria-pressed`/`aria-current` 與 visible focus 清楚表示目前 editor target。`Manage in Library` 必須呼叫既有 `requestDraftAction`／`navigateIfDraftSafe` contract，不可使用繞過 guard 的直接 client-side navigation；tests 覆蓋 dirty draft cancel（停留 Map 且 draft 不變）與 confirm（只導覽一次）。
...
- [ ] 將 Library state 移到 search params，實作既有資料可支援的 search、sort 與 filters：name/notes/tags/mode search；updated/name sort；type/tag/playback-mode filters；空結果顯示 active filters 並可單獨清除或 `Clear all`。每次 direct load、Back/Forward 或 filter/query/type change 都以 filtered results 驗證 selected ID；若不相容，以同一次 atomic URL update 保留新 filter state、移除 selected param並顯示輕量 `Select an item to preview`，direct invalid URL 使用 `replace` 避免污染 history。Back/Forward、reload 與分享 URL 必須恢復同一份 normalized list／preview state；`/dashboard/library/places`、`/dashboard/library/routes` 必須保留為相容頁面，或將已知 search params 安全 redirect 到對應 canonical type state，同時保留 `/dashboard/places`、`/dashboard/routes` 的 legacy redirects。route-level tests 覆蓋四個既有入口、incompatible direct URL、filter 後 selection 排除、bookmark／reload 與目前內部 navigation callers。
...
- [ ] 以 Chrome DevTools 將 1440×900 light mode 作為主要 baseline，並只為本計畫明確要求的 responsive／appearance scope 增加一個 1100×900 panel-budget case、代表性 768×1024 tablet、390×844 mobile 與 1440×900 dark/system checks；驗證 desktop resizer/persistence、persisted-collapse 跨 contextual breakpoint、至少 300 CSS px unobscured map workspace、tablet drawer/context panel、mobile Library list/detail、Account navigation、account-menu quick theme switching、dirty-draft Account links／Sign out／`Manage in Library` cancel／confirm與 account-trigger focus restore、single primary scroll owner、44px common targets、visible focus、no horizontal overflow、reduced motion，以及 Map／Library draft/selection不重設。不要另建 200% zoom 或其他 viewport matrix；截圖保留在 repository 外。
- [ ] 依 PR 邊界交付以降低回歸：先 shared chrome + minimal Account layout／Appearance route + Account menu/theme，次完成 Account overview／Security／Sessions route split，再 Map panel/safe-area/editor polish，最後 Library master-detail/cross-flow + docs；第一個 slice 必須以 route test 與 browser navigation 證明 `Appearance settings` 不會 404、quick mode control 不需離開 workspace、dirty draft 會攔截 account route navigation 與 Sign out，再開始 session revocation，每個 PR 都從當時最新 main 建 branch、只含該 slice、通過受影響 checks，且不得用未完成後續 PR 掩蓋該 slice 的 broken state。
...
- [ ] Map、Library、Account 的角色在 global navigation、頁面 heading、empty/error copy與文件中一致；Map picker不再看起來像第二個管理型 Library，且 `Manage in Library` 的 cancel／confirm evidence 證明 dirty draft 不會被直接 navigation 丟棄。
...
- [ ] Desktop Map panel可收合、鍵盤/指標 resize並安全保存偏好；focusable separator 的 orientation、controlled pane與 current/min/max width可由 assistive technology取得，1024–1150px restore時 joint clamp 後保留至少 300 CSS px unobscured map workspace，Fit route/place和 controls在所有 panel組合下不被遮擋；tablet/mobile忽略 desktop width與collapse flags，Map／Choose／Edit皆可見可操作，返回 desktop後正確恢復。
- [ ] Route identity與 inspector比目前更緊湊，Path／Playback／Save & use可快速導覽；常見 route直接管理 waypoints，50／1000點仍符合已記錄的效能、focus與overflow門檻。
- [ ] Library具有 URL-restorable search/sort/filter/selection、desktop preview、mobile detail flow和明確 `Open on map`；任何 selected ID 都存在於目前 filtered results，不相容 selection 會以 atomic replace 清除且 Back/Forward 不出現矛盾 preview；rows不再重複 Share，也不以模糊箭頭暗示行為，四個既有 Places／Routes URL 入口仍以 route-level tests 證明可恢復正確 type state。
- [ ] 從 Library進入 Map及返回時保留 selected item與 catalog context，dirty Map draft仍受 navigation guard保護。
- [ ] Header account menu不含 password或其他敏感 form，保留不離開 workspace 的 compact 三態 Appearance control，`Appearance settings` link與可用 route在同一 slice交付，且所有 Account links 與 Sign out 都通過 draft action guard；Sign out cancel 保留 draft/session並將 focus 回仍連接的 account-menu trigger，confirm 才執行一次 revocation。Appearance、Security、Sessions & devices位於獨立 Account routes，`/dashboard/account/oidc` callback／exchange 與 `/dashboard/account?oidc=linked` notice 保持相容，現有 OIDC/session/device security契約和focus recovery皆有證據。
- [ ] 沒有虛構 email/avatar/address/device assignment/offline save/undo delete/bulk action；所有 unsupported能力都從 UI與文件排除或明確標示非功能。
- [ ] scoped visual tokens、semantic colors、44px targets、keyboard alternatives、tooltips、non-color state cues與WCAG contrast門檻在 1440×900 light primary baseline，以及本計畫限定的 768×1024 tablet、390×844 mobile、1440×900 dark/system cases 下通過 Chrome evidence。
