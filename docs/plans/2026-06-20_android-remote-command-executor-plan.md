...
- [ ] 擴充 `CloudApiClient` / `CloudModels.kt` 支援 backend remote-control endpoints：register device（送 `clientDeviceId` / `remoteControlEnabled`）、poll command（backend 回傳前標為 `DELIVERED`）、ack command；驗證方式為新增純 Kotlin serialization/unit tests 或 mock client tests，命令 `JAVA_HOME=… ./gradlew :app:testDebugUnitTest --tests '*RemoteControl*'`。
