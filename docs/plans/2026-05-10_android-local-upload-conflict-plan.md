...
- [x] Update backend place create/update/delete flows to bump `LibraryItem.version` for rename, coordinate/content edits, and soft delete; implemented in PR #46, with dedicated version increment tests added later in `backend/src/library/library.service.spec.ts`.
- [x] Add backend sync upload models and `POST /sync/upload` for place create/update/delete changes, with per-item transactions and response buckets `uploaded`, `conflicts`, and `failed`; implemented in PR #46, with partial-success tests added later in `backend/src/sync/sync.service.spec.ts`.
- [x] Add backend idempotency storage keyed by `(userId, clientMutationId)` with request hash and stored result; implemented in PR #46, with retry/reuse tests added later in `backend/src/sync/sync.service.spec.ts`.
...
- [x] Update `CloudSyncRepository` to run pull/upload/pull-confirm and to persist uploaded remote ids/versions back into existing local rows without changing local ids; implemented in PR #46, with Android repository tests added later in `app/src/androidTest/java/dev/narumi/kestrel/core/cloud/CloudSyncRepositoryTest.kt` (currently compile-verified in CI-style local validation; still needs device/emulator execution to count as passing automated Android tests).
- [x] Implement conflict detection for dirty/deleted place changes when backend reports a newer `LibraryItem.version`; persist `sync_conflicts` with local/cloud snapshots and base/remote versions; implemented in PR #46, with backend update-vs-update / delete-vs-update tests added later in `backend/src/sync/sync.service.spec.ts` and Android conflict-persistence coverage added in `CloudSyncRepositoryTest.kt` (compile-verified locally).
...
- [x] Update `docs/plans/archived/2026-05-10_android-local-library-plan.md`, `docs/plans/archived/2026-05-10_android-cloud-sync-plan.md`, and `2026-05-10_product-roadmap-plan.md` checklists to point to this plan and mark place upload/conflict as the active first slice; verified by docs diff in PR #46 follow-up.
- [x] Run full validation: `just check`, `just lint`, backend test/lint commands, and one Android real-device smoke test for local place create → foreground/manual sync → cloud visibility → conflict resolution. See **Validation runbook** below for the exact step sequence.
  ...
  - 2026-05-13 backend follow-up validation: `cd backend && npm test -- --runInBand src/library/library.service.spec.ts src/sync/sync.service.spec.ts`, `cd backend && npm run lint`, and `cd backend && npm run typecheck` all pass after adding version-bump, partial-success, idempotency-reuse, update-success, update-vs-update, and delete-vs-update coverage.
  - 2026-05-13 Android follow-up validation: `just check`, `just lint`, `JAVA_HOME="/Applications/Android Studio.app/Contents/jbr/Contents/Home" ./gradlew :app:compileDebugAndroidTestKotlin`, `JAVA_HOME="/Applications/Android Studio.app/Contents/jbr/Contents/Home" ./gradlew :app:testDebugUnitTest`, and `JAVA_HOME="/Applications/Android Studio.app/Contents/jbr/Contents/Home" ./gradlew :app:connectedDebugAndroidTest` all pass after adding `CloudSyncRepositoryTest.kt`.
  - 2026-05-13 manual smoke follow-up on the same device: Android rename + coordinate edit synced to web successfully; Android delete removed the cloud row after `Sync now` (local UI disappearance was delayed, but the delete propagated); conflict UI appeared after web-vs-Android concurrent rename; `Use Cloud`, `Use Local`, and `Keep Both` all behaved as expected.
  - Known follow-ups out of this run were later reduced by added backend `LibraryItem.version` bump tests, `/sync/upload` partial-success + idempotency reuse tests, richer backend conflict variants (update-vs-update, delete-vs-update), and Android `CloudSyncRepository` instrumentation tests for local-only upload/conflict persistence. Remaining gaps are the unchecked Completion Checklist items below.
...
- [x] Local-only Android places upload to cloud and bind returned `remoteId`/version to the existing local rows, verified by automated tests and an Android smoke test. _Implementation landed in PR #46; 2026-05-11 smoke test on commit `2a0224a` confirmed `smoke-A-1` uploaded and bound `remoteId`/`version`; `CloudSyncRepositoryTest.kt` now executes on-device via `:app:connectedDebugAndroidTest` and covers the local-only upload binding path._
- [ ] Synced Android place rename/coordinate edit/delete syncs to cloud through `/sync/upload`, verified by backend and Android tests. _Backend + Android implementation landed; 2026-05-13 manual smoke now covers Android rename + coordinate edit + delete syncing through to web, and backend `LibraryItem.version` bump / `/sync/upload` update-delete tests are now in place. The remaining gap is dedicated Android automated coverage for synced update/delete execution paths._
- [x] Web/cloud concurrent edits produce persisted item-level Android conflicts, verified by conflict tests and manual UI acceptance. _Detection + persistence implemented; 2026-05-11 smoke surfaced an update-vs-update conflict on Options → Cloud sync; backend update-vs-update / delete-vs-update tests now exist, `CloudSyncRepositoryTest.kt` executes on-device and covers conflict persistence, and 2026-05-13 manual smoke confirmed the conflict UI still appears._
- [x] `Use Cloud`, `Use Local`, and `Keep Both` resolve place conflicts without losing local or cloud snapshots, verified by tests and smoke test. _Actions implemented and covered by `:app:testDebugUnitTest`; 2026-05-11 smoke verified all three end-to-end, and 2026-05-13 manual smoke re-verified `Use Cloud`, `Use Local`, and `Keep Both` all behaved as expected._
- [x] Retry after upload response loss does not duplicate cloud places, verified by idempotency tests. _Backend idempotency storage keyed by `(userId, clientMutationId)` implemented; 2026-05-11 smoke toggled airplane mode mid-upload of `smoke-A-3-IDEM` and the retry produced exactly one cloud place with no crash; backend retry/reuse tests now live in `backend/src/sync/sync.service.spec.ts`._
- [x] Route upload/conflict remains explicitly deferred to a follow-up plan or unchecked roadmap item, verified by docs update. _`2026-05-10_product-roadmap-plan.md`, `docs/plans/archived/2026-05-10_android-local-library-plan.md`, and `docs/plans/archived/2026-05-10_android-cloud-sync-plan.md` already point at this plan as the place-only first slice._
