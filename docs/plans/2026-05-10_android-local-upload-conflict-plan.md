...
- [ ] Local-only Android places upload to cloud and bind returned `remoteId`/version to the existing local rows, verified by automated tests and an Android smoke test. _Implementation landed in PR #46; 2026-05-11 smoke test on commit `2a0224a` confirmed `smoke-A-1` uploaded and bound `remoteId`/`version`; `CloudSyncRepositoryTest.kt` now covers the local-only upload binding path, but it has only been compile-verified so far because no device/emulator-backed Android test run was available in this pass._
- [ ] Synced Android place rename/coordinate edit/delete syncs to cloud through `/sync/upload`, verified by backend and Android tests. _Backend + Android implementation landed; rename round-trip verified on smoke (2026-05-11) for both Android→cloud and cloud-snapshot-wins paths; backend `LibraryItem.version` bump tests and `/sync/upload` partial-success/update/delete tests are now in place, but Android-side update/delete execution coverage and coordinate-edit / delete smoke evidence are still missing._
- [ ] Web/cloud concurrent edits produce persisted item-level Android conflicts, verified by conflict tests and manual UI acceptance. _Detection + persistence implemented; 2026-05-11 smoke surfaced an update-vs-update conflict on Options → Cloud sync; backend update-vs-update / delete-vs-update tests and Android conflict-persistence coverage now exist, but the new Android instrumentation tests have only been compile-verified so far._
- [ ] `Use Cloud`, `Use Local`, and `Keep Both` resolve place conflicts without losing local or cloud snapshots, verified by tests and smoke test. _Actions implemented and covered by `:app:testDebugUnitTest`; all three actions verified end-to-end on the 2026-05-11 smoke test, including Keep Both producing a duplicated local-only row that uploaded as a second cloud place on the next sync; richer repository/sync tests for the resolver paths are still needed._
