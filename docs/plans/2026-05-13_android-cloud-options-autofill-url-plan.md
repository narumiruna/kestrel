...
- [ ] 更新 `CloudSignedOutCardContent` 的 username、password、TOTP/recovery-code 欄位語意，讓 1Password 能辨識帳號、密碼與 one-time code；驗證方式為 `just check`，並以實機 1Password smoke：點選 Username 欄位後可帶入 username，Password 欄位可帶入 password，TOTP 欄位可帶入 OTP 或不阻塞手動輸入。_Attempted semantics and legacy Autofill nodes; both failed manual 1Password smoke, so this remains open and the PR intentionally restores the baseline fields._
