...
- [ ] Add `RuntimeState` sealed interface in `core/location/LocationService.kt` (or a sibling file `core/location/RuntimeState.kt` if it grows) with `Idle`, `Single(point)`, `Route(waypoints, speedKmh, mode, paused)`. Verify with `just check` and by `rg "RuntimeState" app/src/main` showing only the new definition site.
- [ ] Add `private val _runtimeState = MutableStateFlow<RuntimeState>(RuntimeState.Idle)` and `val runtimeState: StateFlow<RuntimeState>` on `LocationService.Companion`, mirroring the existing `currentMock` pattern. Verify with `just check`.
- [ ] Emit on every transition inside `LocationService.onStartCommand` (`ACTION_START_ROUTE`, `ACTION_SET_LOCATION`, `ACTION_PAUSE`, `ACTION_RESUME`, `ACTION_STOP`), inside the Once-finish branch of `startRoute`'s coroutine, and at the end of `restoreState()`. Do **not** emit from the progress writer tick. Verify by unit-testing transitions where feasible, plus inspection.
- [ ] In `MapScreen.kt` replace the `var runState by remember` with a derived value from `LocationService.runtimeState.collectAsStateWithLifecycle()`. Compute `renderedWaypoints` / `renderedSpeedKmh` / `renderedRouteMode` as derived `State` that picks service values when `Route`, otherwise draft values. Pass derived values into `MapSheet`, `KestrelMap`'s `polyline`, and `StatusRow`. Verify by tab-switching during a Loop route on device and seeing the polyline + Pause button restored immediately.
- [ ] Convert the draft locals (`waypoints`, `speedKmh`, `routeMode`) to `rememberSaveable`. For `waypoints: List<LatLng>` add a `listSaver` (`LatLng` is a simple `data class` of two `Double`s). Verify by rotating the device with three drafted waypoints and seeing them still on screen.
- [ ] Update `applyPoint`, `applyItem`, `onClear`, `onMapClick`, and `handlePrimary` to key their "is something running?" decisions off the collected `runtimeState`, not the deleted local `runState`. Keep their existing semantics: replacing a running mock still stops the service first; `onMapClick` only appends to drafts when `runtimeState is Idle` (or `Single` if we choose to preserve current "edit while Single" behavior — preserve as-is to keep the diff small). Verify by code review and by reproducing the original bug to confirm it no longer destroys the route.
- [ ] Update the Once-finish path in `LocationService.startRoute` to also emit `RuntimeState.Single(last)` (it already writes the equivalent `MockState`). Verify by playing a short Once route, switching tabs at the moment it finishes, switching back, and seeing a Single mock pin + Stop button instead of Idle.
- [ ] Add a unit test under `app/src/test/java/dev/narumi/kestrel/feature/map/` that exercises the `runtimeState` → render-state reconciliation as a pure Kotlin function (extract the derive logic so it is testable without Compose / Android context). Verify with `JAVA_HOME=… ./gradlew :app:testDebugUnitTest`.
- [ ] Run `just format`, `just check`, `just lint`, and `JAVA_HOME=… ./gradlew :app:testDebugUnitTest`. Record commit hash + results in this plan.
...
- [ ] Add a `## GOTCHA` entry to `docs/MEMORY.md`: "`LocationService.runtimeState` is the source of truth for any UI that needs to know if a mock is running; never re-derive run state from local Compose `remember` values."
...
- [ ] `LocationService.runtimeState: StateFlow<RuntimeState>` exists, is emitted on every state transition listed in the Plan, and is **not** emitted from the per-tick progress writer, verified by `rg "_runtimeState.value" app/src/main` matching only transition sites.
- [ ] `MapScreen` derives run state from `runtimeState` and no longer reads or writes a local `var runState`, verified by `rg "var runState" app/src/main/java/dev/narumi/kestrel/feature/map/` returning no matches.
...
- [ ] `just format`, `just check`, `just lint`, and `:app:testDebugUnitTest` all pass on the final commit, recorded with commit hash.
- [ ] `docs/MEMORY.md` has the new `## GOTCHA` entry pointing at `LocationService.runtimeState`, verified by `git grep "runtimeState" docs/MEMORY.md`.
