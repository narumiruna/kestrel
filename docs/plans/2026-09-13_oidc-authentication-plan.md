...
- [x] Rename Web API helpers, callback route/component, storage keys, UI state, and labels; bind callback fragments to the initiating authentication attempt, make duplicate recovered sessions idempotent, and display the configured provider name while local login remains available.
- [x] Rename Android models, API helpers, attempt store, callback parser, repository methods, UI state, and messages; isolate the verified callback host from third-party candidate links, persist success tickets before suspending configuration reads, retain callback binding and retry semantics, and keep successful local login and session clearing independent from attempt-cleanup failures.
