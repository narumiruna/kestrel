...
- [x] Update `CloudSyncRepository` to run pull/upload/pull-confirm and to persist uploaded remote ids/versions back into existing local rows without changing local ids; implemented in PR #46, with Android instrumentation coverage later added in `app/src/androidTest/java/dev/narumi/kestrel/core/cloud/CloudSyncRepositoryTest.kt` and executed on-device via `:app:connectedDebugAndroidTest`.
- [x] Implement conflict detection for dirty/deleted place changes when backend reports a newer `LibraryItem.version`; persist `sync_conflicts` with local/cloud snapshots and base/remote versions; implemented in PR #46, with backend update-vs-update / delete-vs-update tests added later in `backend/src/sync/sync.service.spec.ts` and Android conflict-persistence coverage added in `CloudSyncRepositoryTest.kt` and executed on-device.
...
- [x] Run full validation: `just check`, `just lint`, backend test/lint commands, and one Android real-device smoke test for local place create → foreground/manual sync → cloud visibility → conflict resolution. See **Validation runbook** below for the exact step sequence.
  ...
  - Remaining known gaps after these follow-ups are narrowed to richer automated coverage for the resolver paths (`Use Cloud` / `Use Local` / `Keep Both`) rather than the core upload/conflict happy paths.
...
- [x] Synced Android place rename/coordinate edit/delete syncs to cloud through `/sync/upload`, verified by backend and Android tests. _Backend + Android implementation landed; 2026-05-13 manual smoke covers Android rename + coordinate edit + delete syncing through to web, backend `LibraryItem.version` bump / `/sync/upload` update-delete tests are in place, and `CloudSyncRepositoryTest.kt` now executes on-device to cover synced update and delete success paths._
