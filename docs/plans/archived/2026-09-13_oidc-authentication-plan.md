...
- [x] Rename Backend service, routes, wire types, provider key, configuration, logs, and tests from Pocket ID to generic OIDC; 178 unit tests and 9 e2e tests pass.
...
- [x] Atomically claim code callbacks before provider requests, bound claims to 120 new rows per minute and 1,000 active rows, use a bounded processing lease, fetch signing keys before code redemption, retry safe signing-key and UserInfo failures within the callback lease, refresh unknown signing keys after redemption, reject provider redirects, retain claims through both the processing lease and authorization-state expiry, recover completed callbacks after authorization-state expiry, and audit recovered exchanges.
- [x] Derive reproducible callback/session retry secrets, preserve exact issuer, redirect, credential, and endpoint values, support form-correct client authentication, persist Web and Android callback tickets across ambiguous or retryable exchange failures, recover the current credential after refresh rotation, and bound each Web exchange request.
- [x] Rename Web API helpers, callback route/component, storage keys, UI state, and labels; bind callback fragments to the initiating authentication attempt, require secure-context-capable Web callbacks, make duplicate recovered sessions idempotent, and display the configured provider name while local login remains available.
- [x] Rename Android models, API helpers, attempt store, callback parser, repository methods, UI state, and messages; isolate the verified callback host from third-party candidate links, persist success tickets before suspending configuration reads, retain callback binding and retry semantics, invalidate pending OIDC before committing a local login, pin OIDC requests to the initiating server, serialize server changes with authentication, and keep successful authentication and session clearing independent from attempt-cleanup failures.
- [x] Move all deployment-specific OIDC values to GitHub Actions variables, keep secrets in Actions secrets, pass workflow values directly through the Compose process environment, and leave required Compose values empty so OIDC stays optional.
...
- [x] Backend lint, 178 unit tests, 9 e2e tests, typecheck, build, Prisma generation, and schema validation pass.
...
- [x] PR review feedback is refreshed and every new actionable item is addressed before reporting completion.
