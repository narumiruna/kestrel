...
- [x] Rename Backend service, routes, wire types, provider key, configuration, logs, and tests from Pocket ID to generic OIDC; 176 unit tests and 9 e2e tests pass.
...
- [x] Atomically claim code callbacks before provider requests, bound claims to 120 new rows per minute and 1,000 active rows, use a bounded processing lease, fetch signing keys before code redemption, retry safe signing-key and UserInfo failures, refresh unknown signing keys after redemption, reject provider redirects, retain claims through both the processing lease and authorization-state expiry, recover completed callbacks after authorization-state expiry, and audit recovered exchanges.
- [x] Derive reproducible callback/session retry secrets, preserve exact issuer, redirect, credential, and endpoint values, support form-correct client authentication, persist Web and Android callback tickets across ambiguous or retryable exchange failures, and bound each Web exchange request.
...
- [x] Backend lint, 176 unit tests, 9 e2e tests, typecheck, build, Prisma generation, and schema validation pass.
