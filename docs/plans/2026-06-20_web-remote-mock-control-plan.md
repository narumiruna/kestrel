...
- [ ] 實作 `docs/plans/2026-06-20_remote-control-backend-command-queue-plan.md`，提供 authenticated command queue API；驗證方式為 backend unit tests、`cd backend && npm run typecheck && npm run lint && npm run build`。
- [ ] 實作 `docs/plans/2026-06-20_android-remote-command-executor-plan.md`，讓 Android opt-in、註冊 device、poll/ack command、呼叫 `LocationService`；驗證方式為 Android unit tests、`just check && just lint`、非破壞性 manual smoke。
