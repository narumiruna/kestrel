...
The backend validates discovery issuer, ID-token signature, audience, authorized party, expiry, nonce, and subject. Authorization start is stateless: the PKCE verifier, client binding, client type, and expiry travel only in authenticated AES-256-GCM-encrypted state. A callback carrying an authorization code atomically records a state-hash claim with a two-minute processing deadline before contacting the provider, so replaying an in-progress state cannot repeat the outbound token request. Kestrel fetches signing keys before redeeming the one-time authorization code, keeping a transient key outage safely retryable. Provider error callbacks do not contact the provider or write an attempt row. A transient discovery, token, signing-key, or completion-storage failure releases an incomplete claim and returns a retryable server error; terminal provider/code/token-validation failures consume the claim.
