...
Production requires HTTPS. Outside `NODE_ENV=production`, HTTP provider and redirect URLs are accepted for local integration testing; the Web callback may use HTTP only on `localhost`, `127.0.0.1`, or `[::1]` so browser Web Crypto remains available.
